...
**Status:** Active. **Cycle 6 (Audience Routing) implementation complete; not yet closed.** Cycle 5 closed 2026-09-16.
**Started:** 2026-01-31
**Current Phase:** **Cycle 6 (Audience Routing), implementation complete 2026-09-16.** The homepage now routes by visitor intent (Projects / Playground / Résumé doors), with three new destination surfaces, standalone demo back-links, motion on the front door, KaTeX overflow fixed, and an authored description required on every published post. Deployed (`099d03d`). Tracker in `tasks.md`; verification in `ui-checklist.md` Sweep 8. Next focus not yet chosen. **Cycle 5 (Incremental Polish) closed 2026-09-16** without a code review (user decision); curiosity review found no survivors. Shipped: `architecture.md` refresh, Sweep 7, TF-IDF related posts, colophon, and a content batch (resources post, TF-IDF concepts post, related-posts logbook). **Cycle 4 (reader-facing features) closed 2026-06-30.** Shipped: reader text-size/typeface controls (2026-06-24); a persistent sticky header plus Pagefind site search (2026-06-25); post chrome (reading-time, prev/next, breadcrumb JSON-LD, site-wide back-link removal; 2026-06-25); and the **first concepts post**, *How indexing can make search faster* (an interactive inverted-index demo over a Sherlock Holmes corpus, 2026-06-27, with `editorial/concepts.md` updated from the first instance). Privacy/colophon page carried into Cycle 5. *Cycle 3 (Discoverability & UX) closed 2026-06-14: site public-ready (homepage rework, full SEO/AEO plumbing with Lighthouse SEO 100/100, reader accessibility controls, logo/favicon/OG card, GSC/Bing/ahrefs/LinkedIn registration; ahrefs 67 → 98%).*
...
- [x] Cycle 4: Reader-facing features (2026-06-24 → 2026-06-27, closed 2026-06-30). Reader controls, persistent sticky header, Pagefind site search, post chrome (reading-time + prev/next + breadcrumb JSON-LD + back-link removal), and the first concepts post (search-indexing demo). Tracked via plan-mode plans + per-feature review (no `tasks.md` rewrite). **Cycle 4 closed.**
- [x] Cycle 5: Incremental Polish (2026-06-30 → 2026-07-01, closed 2026-09-16). All five tasks shipped: `architecture.md` refreshed, Sweep 7, content-based prev/next via TF-IDF `related.json` (kept over model2vec), the `/colophon/` page, and a content batch (resources post, *Measuring document similarity with TF-IDF*, related-posts logbook). Closed without a Session 6 code review (user decision); curiosity review: no survivors. **Cycle 5 closed.**
- [ ] Cycle 6: Audience Routing (2026-09-15 → 2026-09-16, implementation complete). KaTeX overflow fix, `/resume/` with generated PDF, `/projects/`, derived demo index + standalone back-links, `/playground/`, homepage doors + header nav, hero flock + logo/door motion, required post descriptions + source-rendered preview card. Sweep 8 recorded. Tracked in `tasks.md`.
...
### Cycle 4 — Reader-facing features (2026-06-24 → )

**Goal:** Work through the Cycle 3 deferred reader-facing backlog (site search, reading-time, prev/next nav, first concepts post), serving the founding *"if I can find my own content, others can too"* criterion.

**Tracking note:** Like Cycle 3, tracked via plan-mode plans + per-feature review rather than a `tasks.md` rewrite. `tasks.md` still holds the Cycle 2 list — reset deferred to a formal Cycle 4 Planning session, which this ad-hoc feature work hasn't run.

**Shipped:**

- **Reader text-size & typeface controls (2026-06-24).** The Cycle 3 portfolio-demo candidate realized: first mylearnbase self-documented logbook entry + the site's first `{{ demo() }}`. Published at `content/posts/logbook/mylearnbase/reader-controls/`.
- **Persistent site header (2026-06-25).** Slim sticky bar on every page (reveal-on-scroll-up / tuck-on-scroll-down), nav = `mylearnbase` (home) · Posts · Logbook · Tags, theme toggle consolidated into it (footer duplicate removed via a `_footer.html` override), inert search-trigger stub. Required a project override of the theme's `_base.html` (Serene ships no body-top hook → we now own `_base.html`; theme pinned v5.6.1 — re-merge on future bumps). Homepage recomposed: removed the redundant `#left` nav row (guide cards are now the sole on-page discovery surface), folded social icons into the profile block, swapped `extra.sections` for a new `extra.nav`. New `static/js/header.js` (rAF-throttled scroll-direction). Nav font-size bug fixed (em-compounding: theme `header{0.8em}` × inner). Commit `b079fe4`.
- **Pagefind site search (2026-06-25).** Post-build static search: new `build.sh` runs `zola build` then `npx pagefind --site public` (index → `public/pagefind/`); `data-pagefind-body` on `article.prose` scopes indexing to post bodies. Overlay UI (`static/js/search.js`) reveals the header trigger, **lazy-loads** the Pagefind UI bundle on first open, opens on click / `/` / Cmd-Ctrl-K, Esc/backdrop close, focus-trap, fail-soft if the index is missing. Themed via `--pagefind-ui-*` → theme vars (light + dark). **Engine choice (verified):** static/no-backend rules out hosted engines (Algolia/Meili/Typesense); of the in-browser options Pagefind is the maintained one and indexes *rendered HTML* (so it covers aggregated `logbook/<project>/`). Zola's native `build_search_index` stays off. **User action (done):** Cloudflare Pages build command → `bash build.sh` (`docs/src/reference/cloudflare.md` updated). Commit `19c11d9`. First production build indexed cleanly.
- **Two meta-logbook entries (2026-06-25).** `site-header` + `site-search` under `content/posts/logbook/mylearnbase/`, cross-linked, each with live-site observable + cite-block evidence; published live (draft=false). Caught + fixed a render bug — literal `<body>` / `logbook/<project>/` in cite notes were parsed as HTML and swallowed §7 into an unclosed blockquote; fix = wrap any tag-shaped text in backticks (cousin of the known `{{ }}`-in-code-blocks gotcha). Index now 25 pages (the two new entries are themselves searchable).
- **Post chrome + navigation cleanup (2026-06-25).** In `templates/post.html` + `static/css/custom.css`: **reading-time** (`page.reading_time`, pale, beside the date) and **prev/next** adjacent-post nav. Prev/next is project-scoped by walking the parent section's date-sorted `.pages` — the project sections are `transparent`, so Zola drops `page.earlier`/`.later` AND omits them from `page.ancestors`; the parent section is therefore derived from `page.components`. **Breadcrumbs ship as `BreadcrumbList` JSON-LD only** (`_head_extend.html`, beside `BlogPosting`): a visible trail was built first, then cut (too small; and reusing `id=back-link` on the project crumb made it `history.back()` off-target). Then **removed the in-content `← back` links site-wide** (post + posts_aggregator/blog + tags + series): the persistent header + browser Back supersede them, and the theme's `enableBackLink()` mis-fired whenever the referrer was the home page (its URL is a prefix of every page) — now dead code (left untouched; theme submodule). Same-day tie fixed by timestamping `site-search` `date = …T12:00:00` so prev/next orders truthfully — **`logbook publish` stamps date-only, so any two same-day posts will tie until the tool stamps a time** (systemic follow-up).

**Phase D — Post-system pulls (curiosities):** `.curiosities/cycle-4.md` seeded — (1) **how site search actually works** (server engine vs in-browser JS index vs post-build rendered-HTML indexing; strong concepts-demo candidate — user flagged a real "gap between my understanding and reality"), (2) why `em` sizing compounds down the DOM tree, (3) the **3blue1brown-style programmatic-visualization** demo direction (→ memory `project_concepts_demos_visualization_library`).

**Remaining backlog:** first concepts post (the "how search works" demo, ideally a programmatic visualization — see memory `project_concepts_demos_visualization_library`). **Parked:** privacy/colophon page — site collects nothing (functional `localStorage` only; ePrivacy "strictly necessary" → no policy/banner legally required); couple to a future *cookieless* analytics decision. *(Carried into Cycle 5 as a scoped task.)*

---

### Cycle 5 — Incremental Polish (Planning, 2026-06-30)

**Goal:** A deliberately small cycle of "little things," no sweeping changes. Bring drifted project docs back in line with reality, replace arbitrary chronological prev/next with content-based relatedness, ship the parked privacy page, and publish a content batch about the site itself.

**Why now:** Cycle 4's headline goal (first concepts post) shipped 2026-06-27, so the reader-facing backlog is essentially cleared. `tasks.md` was two cycles stale (still the completed Cycle 2 list, since Cycles 3-4 tracked via plan-mode plans). This session resets `tasks.md` and resumes tasks.md-based tracking.

**Cycle 5 tasks** (full detail + rationale in `tasks.md`):

1. **Refresh `architecture.md`.** It had drifted badly: untouched since 2026-02-02, still describing the abandoned Dioxus stack, a `blog/` + `config.toml` layout, and "search deferred." Rewrite to the real Zola / Serene / Pagefind / Cloudflare reality.
2. **Verification sweep (Sweep 7) → update `ui-checklist.md`.** The checklist predated Cycle 4, so it still marked reading-time / prev-next / search as "not yet built." Re-sweep and bring current.
3. **Prev/next redesign.** Move off chronological ordering (arbitrary) to content-based relatedness. Decided: hand-built TF-IDF (pure Python, `scripts/compute-related.py`), computed in CI via a new `build.sh` step, read by `templates/post.html` through a gitignored `related.json` seam, with a chronological fallback for unindexed posts. model2vec static embeddings are the seam-swappable upgrade if TF-IDF matches read weak. Subsumes the Cycle-4 "same-day prev/next tie" follow-up. Python chosen over Rust because the model2vec upgrade target is a Python/numpy library.
4. **Privacy / colophon page.** The parked Cycle-4 item.
5. **Content batch.** A resources post ("Resources used to build mylearnbase," the first post in the empty resources section); the queued search-ranking / TF-IDF concepts candidate (dependency now satisfied since the indexing post landed); and reader-facing logbook candidates (the new prev/next and the privacy page qualify; the doc-refresh tasks do not, per the reader-facing-only meta-logbook rule).

**Process note:** Lightweight cycle. The only design-bearing item (prev/next) was settled in this planning conversation via a researched lexical-vs-semantic comparison; the rest are clear, small tasks. Plan archived at the plan-mode plan file. Two curiosities captured in `.curiosities/cycle-5.md` (lexical-vs-semantic similarity; where build-time computation should live).

**Implementation sitting (2026-06-30, same day). Completed Tasks 1, 2, 3, and 4.**

*Task 1, `architecture.md` refreshed.* Verified every claim against the live tree first, then rewrote: the stack table (Zola 0.22.1, Serene v5.6.1 submodule, Pagefind 1.5.2, Cloudflare via `bash build.sh`, mdBook on GitHub Pages), the real `content/posts/<form>/` five-form layout (replacing the dead `content/blog/` tree), the full template-override surface, and the deployment section (fixing the false "Cloudflare auto-detects, no build command" claim). Swapped the never-built WASM-islands section for the actual `{{ demo() }}` iframe mechanism and appended a Cycle-5 revision-history entry.

*Task 3, content-based prev/next shipped.* `scripts/compute-related.py` builds pure-stdlib TF-IDF vectors over title + headings + body (never tags, deliberately) and writes top-K neighbours to a gitignored `related.json`. `build.sh` regenerates it before `zola build`; `post.html` reads it via `load_data(required=false)` and renders the top 2 as a "Related" nav (resolving each neighbour path with `get_page`), falling back to the prior chronological nav when the artifact is absent (the dev/prod split already used for search). **Decision gate: matches read strong on the 26-post corpus, so staying on TF-IDF; model2vec not needed.** That confirms the single-author / consistent-vocabulary hypothesis from `.curiosities/cycle-5.md`. The model2vec upgrade remains a documented drop-in if the corpus ever diversifies. Verified end to end: `bash build.sh` exit 0, `zola check` clean, both related and fallback paths render correctly.

*Task 2, verification Sweep 7.* Ran the full audit (`zola build` + `zola check` + `scripts/seo-audit.sh`, all clean, Lighthouse SEO 100/100) plus a Playwright browser pass (1280px + 375px, light/dark, console capture) and rendered-HTML grep. Brought `ui-checklist.md` current: flipped reading-time, Pagefind search, reader controls, breadcrumb JSON-LD, and the new content-based prev/next to verified; honestly left KaTeX/Mermaid/`superseded_by` unchecked (wired but no content exercises them) plus focus-states/keyboard-nav/CLS (not measured). Browser pass: 0 console errors/warnings, no horizontal overflow at either width, dark-mode contrast 9.47:1, theme persists across navigation. **Bonus fix:** `seo-audit.sh`'s JSON-LD check was false-failing on 26 valid Cycle-4 `BreadcrumbList` blocks (allowlist predated them); taught it `BreadcrumbList` so the gate is trustworthy again. Benign finding logged: a redirect-alias stub (`/posts/autonomous-ui-development-with-playwright-mcp/`) has no `<html>` element, expected for Zola alias shells.

*Task 4, privacy/colophon page shipped.* A single combined **Colophon** page at `/colophon/` (`content/colophon/_index.md` as a one-off Serene prose section), with a "How it's built" section (the Zola/Serene/Pagefind/Cloudflare/mdBook stack plus the demo-iframe and TF-IDF "Related" mechanisms) and a "Privacy" section (no analytics/tracking/cookies; only functional `localStorage`/`sessionStorage` prefs; in-browser search; the math/diagram CDN caveat; no consent banner needed). Wired into the footer via `get_url(path='@/colophon/_index.md')` (build-validated), so it appears on every content page; not on the home landing, which has `footer = false` by theme design. Deliberately no link yet to the not-yet-existing Task-5 resources post (would break `zola check`); the colophon notes that post is in the works. Needed `[extra] title`/`subtitle` for the visible masthead — Serene renders the on-page `<h1>` from `extra.title`, not the top-level `title`. Verified: build + `zola check` clean, page renders with title/subtitle/both sections, footer link resolves.

**Content sitting (2026-06-30, after export + compact). Task 5 begun — resources post published.** Authored and **published** the first resources post, *Resources used to build mylearnbase* (`content/posts/resources/building-mylearnbase.md`), collaboratively over several review passes. Project-derived by-act curation per `editorial/resources.md`: seven sections of tools actually used, plus an *Alternatives considered* section (theme/generator comparison + the abandoned Rust-fullstack PoC) **sourced from the session logs** after the user confirmed there was no standalone external reading — the research lived inside Claude Code sessions. Factual/objective tone, kept deliberately (user: documents what was used and considered, as a starting point for similar builds). Editorial decisions surfaced and settled in-conversation: dropped the internal "Cycle 5" reference from the summary (reader-facing-vocab discipline — new memory `feedback_no_internal_vocab_in_posts`); reframed a mislabelled "background reading" section to *Alternatives considered* once it was clear the content was comparison artifacts, not reading; moved `schema.org` to SEO where it's a used reference. All 26 external links verified live; post integrated into `related.json` (top neighbour: the MVP build-it logbook post) and Pagefind (27 pages). Sync upkeep left to the section's 180-day `outdate_alert` banner (user choice over memory/pointer machinery). **Also fixed** a user-reported colophon `← Back` bug via a `templates/prose.html` override (the first prose page exposed a back-link Cycle 4's removal never reached) and wired the colophon to link the now-published resources post.

**Content sitting (2026-06-30 → 2026-07-01, across several export/compact parts). Related-posts logbook + second concepts post published — Task 5 complete.** Shipped the *Content-based related posts* logbook entry (2026-06-30) and then the second concepts post, *Measuring document similarity with TF-IDF* (`content/posts/concepts/tf-idf.md`, published 2026-07-01). The post teaches TF-IDF through three interactive demos over a dense Wikipedia corpus — an **ablation** (why TF and IDF need each other: count → filler, rarity → noise, the product → topic words), a **whole-document cosine comparison** (per-word contribution breakdown that sums to the score, ×0 unique words, and a neighbour-ranking panel), and a **bring-your-own-text** tool (paste two texts, live cosine, blank-by-default with example pairs). Two research-backed framings are woven in with citations: near-orthogonality of high-dimensional sparse vectors (read cosine as *ranking*, not magnitude — [Ni et al. 2024](https://arxiv.org/abs/2401.00422)) and the vocabulary-mismatch problem (why lexical similarity caps out and embeddings exist — [Furnas et al. 1987](https://dl.acm.org/doi/10.1145/32206.32212)). The corpus + demo shape were the real work: iterated to a dense set (planets / programming languages / big cats / string instruments) so within-cluster relatedness reads cleanly, and rebuilt the comparison demo from a rejected 2-word-plane projection to a whole-document view on author direction. Bidirectional cross-links wired (indexing post ↔ this ↔ related-posts logbook), and the post's own "Related" links resolve to those two companions via `related.json` (the feature dogfooding the concept it teaches). Review round: descriptive title (dropped the "obnoxious" framing), de-narrated opening, author-written "What clicked" (LLM-polished for grammar only). **Site fixes alongside:** the `.demo--wide` breakout now matches the content column on TOC screens instead of skewing left (Serene's TOC fills the entire right gutter, so a centred wide breakout can't clear it — verified across how-search-works + reader-controls, no regression); `scripts/compute-related.py` gained the same ordinal-fragment (`th`/`st`/`nd`/`rd`) tokenizer filter the demos use (was leaving `th` from "19th"/"20th"); and `editorial/concepts.md` was cleaned up (specific-post citations confined to the worked example, the post-backlog paragraph removed since the editorial isn't the home for future-post ideas — those live in the curiosity logs + `project.md` — and stale "n=1 / one post" facts fixed now that a second post exists).

**Cycle 5 content batch complete.** All of Task 5 shipped (resources + concepts + related-posts logbook), on top of Tasks 1–4 (doc refresh, verification sweep, content-based relatedness, colophon). What remains for a formal cycle close is the Session-6 review pass: code review of the cycle's changes and the `.curiosities/cycle-5.md` survivor review (whose surviving curiosities seed the next cycle's concepts candidates) — to be opened when the user is ready.

**Content sitting (2026-07-11, free session outside the Cycle-5 batch; closeout completed 2026-07-12). Spontaneous concepts post *How SVGs actually work* published.** A standalone concepts post authored outside the already-complete Task-5 content batch: *How SVGs actually work* (`content/posts/concepts/how-svgs-work.md`, slug `how-svgs-work`, `date = 2026-07-11`, committed `3a2559c`). It teaches SVG internals through three interactive demos under `static/demos/concepts/how-svgs-work/`: **scale** (raster pixels blur on zoom while vector instructions stay crisp, the promise hiding in the "S"), **path** (how a `<path>` is defined as coordinates on a Cartesian plane), and **build** (assembling a shape from primitives). The author wrote both personal-voice sections (*Where the question started*, *What clicked*); per the guardrail `feedback_never_draft_curiosity_moment`, the curiosity-moment section was left entirely to the author, with the LLM only tightening grammar and run-ons on the approved drafts (no ideas added or cut). The descriptive prose got a de-AI pass drawn from the parked editorial-voice initiative `project_editorial_voice_initiative`: four setup-then-payoff colon tells re-homed to periods and a filler "start to finish" trimmed (em-dashes were already clean). Ship gate before commit: 0 em-dashes, no leftover placeholders, `draft = false`, clean non-draft build (30 pages, 0 orphan). The sitting itself did not close cleanly: the post shipped and the parent auto-synced immediately (`44ae89a`), but this Session-Log entry plus the submodule and parent pushes were finished in a 2026-07-12 follow-up.

**Between cycles (2026-08):** two concepts posts were published from the boids project rather than as Cycle 5 work: *A Flock Is a Control Loop* (2026-08-03) and *Two Languages, Two Bottlenecks* (2026-08-22). The first set `extra.math`, which later exposed the KaTeX overflow fixed in Cycle 6.

**Cycle 5 close (2026-09-16).** Every task had been done since 2026-07-01, but the cycle was never formally closed. Closed without a Session 6 code review: the user chose to close it the way Cycles 3 and 4 were closed and to leave no review on the schedule, rather than review Cycle 5 now or fold it into Cycle 6's close. Phase D: `.curiosities/cycle-5.md` had two entries (lexical vs semantic similarity; where build-time computation should run), and neither survived. The first is largely covered by the published TF-IDF post; the second is a design trade-off with nothing obvious to demo. Both had been logged by Claude mid-design, which the 2026-09-04 capture rule no longer allows. **Cycle 5 closed.**

---

### Cycle 6 — Audience Routing (2026-09-15 → 2026-09-16)

**Goal:** Route a first-time visitor by intent rather than by post form. The front door was organised by the author's taxonomy (logbook, concepts, workflows, opinions, resources), and the three intents that arrive (recruiter, project browser, explorer) had no surface at all: no résumé, nothing project-shaped, and 11 demos reachable only through the post embedding each one. Source: two items the user flagged 2026-08-16 (landing-page wayfinding, KaTeX overflow) that sat in a memory note and were never filed here.

**Tracking note:** Run from a plan-mode plan (`~/.claude/plans/could-you-look-into-abstract-kahn.md`), like Cycles 3 and 4. `tasks.md` was reset to the Cycle 6 list at the end. Design rationale lives in `architecture.md`; verification in `ui-checklist.md` Sweep 8.

**Shipped** (all verified at 1280px and 375px, light and dark):

- **KaTeX overflow.** Three display equations on *A Flock Is a Control Loop* added 162px of sideways page scroll on phones. Scrolled inside the equation instead, with a shadow cue because iOS hides scrollbars. It survived Sweep 7 because that sweep filed a 1280px measurement under the 375px row.
- **Résumé `/resume/`.** One template for page and PDF. Markdown was tried first and rejected: its PDF ran three pages and read like a web page, so the content became structured records. The PDF is generated locally and committed, not built in CI, because Cloudflare's image isn't guaranteed a browser; `build.sh` warns on drift. No email until an efeerhie.com alias exists; phone and address never.
- **Projects `/projects/`.** Per-project pages over a single data file, so each project has a URL a recruiter can link. Only facts that can't be derived are authored; write-ups and demos are derived from published posts.
- **Demo index, back-links, Playground.** `demos.json` is derived from the `demo()` calls rather than hand-authored, so it cannot drift and reports broken embeds. Standalone demos gained a back-link bar that stays out of embedded iframes. `/playground/` rather than `/demos/`, because Zola's static copier owns `public/demos/`. The GIFs proposed as posters were rejected (heavier than the WASM they would replace); light demos preview live, heavy ones use content-addressed posters that cannot go stale.
- **Homepage and header.** Doors (Projects / Playground / Résumé), a dated Now line, Latest 5; the form guide moved to `/posts/`. Nav is Projects · Playground · Posts · Résumé, with Posts hidden below 575px, a measured breakpoint that replaced the mockup's 760px.
- **Motion.** A hero flock ported from the boids Rust core (~4.7KB gzipped against the 533KB WASM demo), sized by density rather than count, with no click-through (a tap to scroll would navigate). Logo hover and door stagger in pure CSS. Everything sits behind `prefers-reduced-motion: no-preference`. Lighthouse perf 99, CLS 0.
- **Post descriptions.** No post had ever set one; two fallback chains shipped generic text and disagreed. Now one chain, and a published post without a description fails the build (fail, not warn, because publishing introduces the gap). Deriving descriptions from the first paragraph was rejected, since concepts posts open in personal voice. Publish tools take, keep and require `--description`. All 32 posts backfilled. The link-preview card is now rendered from source.

**User decisions:** tagline, `/posts/` subtitle, Now line and form copy approved; all 32 descriptions approved as drafted; motion approved; `skip-the-boilerplate.md` (an unfinished opinions draft) gitignored rather than pushed.

**Deploy incident, same day.** Four pushes failed on Cloudflare. To stop the session hook fast-forwarding the theme to v6.0.0, `.gitmodules` had been given `update = none`, which every clone obeys, so Cloudflare built without a theme. Replaced with `pinned = true`, a key only the hook reads; verified with a fresh clone and a full `build.sh` before pushing (`099d03d` deployed). `update = none` also persists in any clone's local config once `submodule init` has run, so the hook's recovery advice keeps `--checkout`.

**Open, unscheduled:** meta descriptions on the 12 standalone demo pages; the front-matter template body describes the old post format; résumé polish; demos ignore the site's theme toggle.
