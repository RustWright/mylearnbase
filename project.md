...
- [ ] Cycle 5: Incremental Polish (2026-06-30, in progress). Planning: `tasks.md` reset + project-doc transition. Implementation so far: **Task 1** (`architecture.md` refreshed), **Task 3** (content-based prev/next via TF-IDF `related.json`; decision gate kept TF-IDF over model2vec), **Task 2** (verification Sweep 7 → `ui-checklist.md` current; also fixed a stale `seo-audit.sh` JSON-LD allowlist), and **Task 4** (combined privacy/colophon page at `/colophon/`, footer-linked) done 2026-06-30. **Task 5** begun: first resources post *Resources used to build mylearnbase* **published** 2026-06-30 (+ colophon cross-link and a `prose.html` back-link fix); the two concepts/logbook *candidates* remain optional. Tracked in `tasks.md`.
