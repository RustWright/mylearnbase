...
**Remaining backlog:** reading-time, prev/next post nav, first concepts post (the "how search works" demo is the leading candidate, ideally a programmatic visualization). **Parked:** privacy/colophon page — site collects nothing (functional `localStorage` only; ePrivacy "strictly necessary" → no policy/banner legally required); couple to a future *cookieless* analytics decision.
